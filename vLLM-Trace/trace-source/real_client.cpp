...
// ============================================================================
// R4 TTFT INSTRUMENTATION -- NATIVE SSD PIPELINE TRACE OUTPUT
// Emits one JSONL record per native phase. Python emits request-to-key mappings;
// Run-all correlates these native key lists back to the originating R4 request.
// ============================================================================
std::string r4_trace_escape(const std::string &value) {  // pingtao add
    std::string escaped;  // pingtao add
    escaped.reserve(value.size());  // pingtao add
    for (char ch : value) {  // pingtao add
        switch (ch) {  // pingtao add
            case '\\': escaped += "\\\\"; break;  // pingtao add
            case '"': escaped += "\\\""; break;  // pingtao add
            case '\n': escaped += "\\n"; break;  // pingtao add
            case '\r': escaped += "\\r"; break;  // pingtao add
            case '\t': escaped += "\\t"; break;  // pingtao add
            default: escaped += ch; break;  // pingtao add
        }  // pingtao add
    }  // pingtao add
    return escaped;  // pingtao add
}  // pingtao add

void write_r4_trace(const char *event, const std::vector<std::string> &keys,  // pingtao add
                    double elapsed_ms, bool success) {  // pingtao add
    const char *trace_path = std::getenv("VB_KV_TRACE_FILE");  // pingtao add
    if (trace_path == nullptr || trace_path[0] == '\0') return;  // pingtao add
    std::ostringstream payload;  // pingtao add
    payload << "{\"event\":\"" << event << "\",\"pid\":" << getpid()  // pingtao add
            << ",\"elapsed_ms\":" << elapsed_ms  // pingtao add
            << ",\"success\":" << (success ? "true" : "false")  // pingtao add
            << ",\"keys\":[";  // pingtao add
    for (size_t index = 0; index < keys.size(); ++index) {  // pingtao add
        if (index != 0) payload << ',';  // pingtao add
        payload << '"' << r4_trace_escape(keys[index]) << '"';  // pingtao add
    }  // pingtao add
    payload << "]}\n";  // pingtao add
    const std::string line = payload.str();  // pingtao add
    const int fd = ::open(trace_path, O_WRONLY | O_CREAT | O_APPEND, 0644);  // pingtao add
    if (fd < 0) return;  // pingtao add
    const ssize_t written = ::write(fd, line.data(), line.size());  // pingtao add
    (void)written;  // pingtao add
    ::close(fd);  // pingtao add
}  // pingtao add
...
RealClient::batch_get_into_multi_buffers_internal(
    ...
    if (!valid_local_disk_ops.empty()) {
        ...
        // (storage_backend::vector_read cannot write directly to GPU memory)
        {
            ...
            if (!disk_batch_keys.empty()) {
                const auto ssd_to_dram_start = std::chrono::steady_clock::now();  // pingtao add
                auto disk_results = client_->BatchGet(
                    disk_batch_keys, disk_batch_qrs, disk_batch_slices,
                    prefer_alloc_in_same_node);
                const auto ssd_to_dram_end = std::chrono::steady_clock::now();  // pingtao add
                const double ssd_to_dram_ms =  // pingtao add
                    std::chrono::duration<double, std::milli>(ssd_to_dram_end - ssd_to_dram_start).count();  // pingtao add
                const bool disk_read_success = std::all_of(  // pingtao add
                    disk_results.begin(), disk_results.end(),  // pingtao add
                    [](const auto &result) { return result.has_value(); });  // pingtao add
                write_r4_trace("ssd_to_dram", disk_batch_keys, ssd_to_dram_ms, disk_read_success);  // pingtao add

                bool scatter_success = true;  // pingtao add
                const auto dram_to_hbm_start = std::chrono::steady_clock::now();  // pingtao add
                for (size_t di = 0; di < disk_key_order.size(); ++di) {
                    ...
                    if (!disk_results[di]) {
                        ...
                        scatter_success = false;  // pingtao add
                        continue;
                    }
                ...
                const auto dram_to_hbm_end = std::chrono::steady_clock::now();  // pingtao add
                const double dram_to_hbm_ms =  // pingtao add
                    std::chrono::duration<double, std::milli>(dram_to_hbm_end - dram_to_hbm_start).count();  // pingtao add
                write_r4_trace("dram_to_hbm", disk_batch_keys, dram_to_hbm_ms, scatter_success);  // pingtao add
            }
...
RealClient::batch_get_offload_object(const std::vector<std::string> &keys,
    ...
    const double ssd_to_dram_ms =  // pingtao add
        std::chrono::duration<double, std::milli>(ssd_to_dram_end - ssd_to_dram_start).count();  // pingtao add
    write_r4_trace("ssd_to_dram", s->keys, ssd_to_dram_ms, result.has_value());  // pingtao add
    if (!result) {
...
RealClient::batch_get_into_offload_object_internal(
    ...
    const double dram_to_hbm_ms =  // pingtao add
        std::chrono::duration<double, std::milli>(dram_to_hbm_end - dram_to_hbm_start).count();  // pingtao add
    write_r4_trace("dram_to_hbm", keys, dram_to_hbm_ms, result.has_value());  // pingtao add
    auto end_time = std::chrono::steady_clock::now();
